...
# ─────────────────────────────────────────────────────────────
# Multi-User Long-Form Hindi Video Pipeline Runner
# ─────────────────────────────────────────────────────────────

def run_long_pipeline_for_user(
    user:    User,
    channel: YouTubeChannel,
    db:      Session,
    job_id:  str | None = None,
    ...
    """
    Run the 16:9 1080p long-form Hindi video generation pipeline for one user.
    """
    from autoshorts.services.trend_sources   import collect_worldwide_trends
    from autoshorts.services.gemini_service  import generate_long_video_topics, generate_long_hindi_video_plan, research_topic
    from autoshorts.services.voice_service   import generate_voice
    from autoshorts.services.renderer        import render_long_video
    from autoshorts.long_video_pipeline      import _collect_landscape_clips
    ...
    user_dir   = PROJECT_ROOT / "downloads" / f"{user.id}_long"
    output_dir = PROJECT_ROOT / "output"    / f"{user.id}_long"
    thumb_dir  = PROJECT_ROOT / "thumbnails"/ f"{user.id}_long"
    user_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    thumb_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        check_cancelled()
        used = _get_used_topics(user.id, db)
        ideas = generate_long_video_topics(trends, count=5, used_topics=used)
        if not ideas:
            raise RuntimeError("Gemini returned no long video topics")

        selected = next((i for i in ideas if not _topic_exists_for_user(user.id, i.topic, db)), ideas[0])
        topic = selected.topic
        style = selected.style
        ...
        # Step 4: Video Plan
        check_cancelled()
        plan = generate_long_hindi_video_plan(topic=topic, style=style, research_summary=research)
        ...
        generate_voice(
            text        = plan.script,
            output_path = audio_path,
            voice       = plan.voice or "hi-IN-MadhurNeural",
            language    = "Hindi",
            style       = plan.style,
        )
        ...
        # Step 10: Upload to YouTube (no #Shorts)
        youtube_video_id = _upload_long_to_youtube(
            plan       = plan,
            video_path = video_path,
            channel    = channel,
            privacy    = user.default_privacy or "public",
            db         = db,
        )
...
def _upload_long_to_youtube(
    ...
    """Upload full-length 16:9 video to YouTube (no #Shorts)."""
    from backend.auth_service import decrypt_token
    from backend.routers.channels import refresh_channel_tokens
    from google.oauth2.credentials import Credentials
    from googleapiclient.discovery import build
    ...
    refresh_token = decrypt_token(channel.refresh_token_enc or "")
    if not access_token and not refresh_token:
        raise RuntimeError("YouTube channel has no valid tokens.")
    ...
    description = plan.description
    guaranteed_tags = ["hindi", "viral", "trending", "explore", "knowledge"]
    plan_tags = [t.strip().lstrip("#") for t in plan.hashtags if t.strip()]
    all_tags  = list(dict.fromkeys(guaranteed_tags + plan_tags))[:25]

    hash_str = " ".join(f"#{t}" for t in all_tags)
    description += f"\n\n{hash_str}"
